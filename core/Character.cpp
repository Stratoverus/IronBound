...
Character::Character()
    : name(""), charClass(""), health(0), baseDamage(0), defense(0), speed(0), xp(0), level(1), spriteFolder("") {}

Character::Character(const std::string& n, const std::string& c, int h, int bd, int d, int s, int x, int lvl,
                     const std::vector<std::string>& inv, const std::vector<std::string>& eq, const std::string& spriteFolder)
    : name(n), charClass(c), health(h), baseDamage(bd), defense(d), speed(s), xp(x), level(lvl), inventory(inv), equipped(eq), spriteFolder(spriteFolder) {}
